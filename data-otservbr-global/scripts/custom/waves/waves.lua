...
function event.spawnWave(wave)
    ...
    for _, monster in ipairs(config.monsters[wave]) do
        local randomPosition = Position(
            ...
            math.random(upperCorner.y, lowerCorner.y),
            math.random(upperCorner.z, lowerCorner.z)
        )
...
local globalEvent = GlobalEvent("MonsterWaveEvent")
globalEvent:registerEvent("MonsterWaveEvent", event.onThink)
globalEvent:registerEvent("MonsterWaveEvent", event.onThink, EVENT_INTERVAL)
