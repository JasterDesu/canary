local config = {
    minimumLevel = 500,  -- Nível mínimo necessário
    actionId = 44500    -- Substitua pelo valor da Action ID desejada
}
...
function leveltopass.onStepIn(player, item, position, fromPosition)
    ...
    if player:getLevel() < config.minimumLevel then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, "Voce precisa ter level 500 para passar.")
